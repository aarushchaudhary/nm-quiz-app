...
<div class="manage-container">
    ...
    <div class="section-box control-panel">
        ...
        <p>Current Status: <strong style="font-size: 18px;"><?php echo htmlspecialchars($quiz['status_name']); ?></strong></p>
        <form action="/nmims_quiz_app/api/faculty/update_quiz_status.php" method="POST">
            <input type="hidden" name="quiz_id" value="<?php echo $quiz_id; ?>">
            <div class="button-group">
                <?php if ($quiz['status_name'] == 'Not Started'): ?>
                    <button type="submit" name="new_status_id" value="2" class="btn-open-lobby">Open Lobby</button>
                <?php elseif ($quiz['status_name'] == 'Lobby Open'): ?>
                    <button type="submit" name="new_status_id" value="3" class="btn-start-exam">Start Exam Now</button>
                <?php elseif ($quiz['status_name'] == 'In Progress'): ?>
                    <button type="submit" name="new_status_id" value="4" class="btn-end-exam" onclick="return confirm('Are you sure you want to end the exam for all students? This cannot be undone.');">End Exam Now</button>
                <?php else: ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // **FIX:** Use event delegation for click handling on the table body
    studentListBody.addEventListener('click', async function(e) {
        // Check if a re-enable button was clicked
        if (e.target && e.target.classList.contains('btn-reenable')) {
            ...
            if (confirm('Are you sure you want to re-enable this student?')) {
                ...
                try {
                    ...
                    fetchMonitoringData(); // Refresh the list immediately
                } catch (error) {
